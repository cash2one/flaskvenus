...
def init_user():
    ...
    user = User(uin=101, name='admin101', phoneNO='13760480101', _password=generate_password_hash('123456'), role=User.ADMIN, avatarId='http://120.24.208.105/venus/api/v1/image/header1')
    user.save()
    user = User(uin=102, name='admin102', phoneNO='13760480102', _password=generate_password_hash('123456'), role=User.ADMIN, avatarId='http://120.24.208.105/venus/api/v1/image/header2')
    user.save()
    user = User(uin=103, name='admin103', phoneNO='13760480103', _password=generate_password_hash('123456'), role=User.ADMIN, avatarId='http://120.24.208.105/venus/api/v1/image/header3')
    user.save()
    
def init_tag():
    tag = Tag(name='榜单', createuin=101, scope='of', subject = 'topic')
    tag.save()
    tag = Tag(name='今日推荐', createuin=101, scope='of', subject = 'topic')
    tag.save()
    tag = Tag(name='小编推荐', createuin=101, scope='of', subject = 'topic')
    tag.save()
    ...
    tag.save()
    tag = Tag(name='看美女', createuin=101, scope='pu', subject = 'scenic')
    tag.save()
    tag = Tag(name='看球赛', createuin=101, scope='pu', subject = 'scenic')
    tag.save()
